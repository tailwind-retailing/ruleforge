...
- **New node templates** (e.g., a curated `call endpoint` template under the existing `api` category). All declared `NodeCategory` enum values are now implemented in the switch at `RuleRunner.cs:308` — `api` shipped via #3, `sql` was removed via #4 since DocumentForge already covers reads. Separate concern: `product` is implemented but its name is misleading — the implementation is output-shape assembly, not Cartesian product (see GitHub issue #5). New domain-flavored templates (e.g. `call endpoint`) are out of scope for THIS pass — see "Schema contract handoff" for the flow.
